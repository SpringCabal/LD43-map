...
vec4 GetSplatDiffuse(vec4 diffuseCol, vec2 uv, out vec2 splatDetailStrength) {
	vec4 splatTexCoord0 = vertexWorldPos.xzxz * splatTexScales.rrgg;
	vec4 splatTexCoord1 = vertexWorldPos.xzxz * splatTexScales.bbaa;
	vec4 splatCofac = texture2D(splatDistrTex, uv) * splatTexMults;
	...
	// 							  +
	// 							  * weights.g +
	// 							 texture2D(splatDetailNormalTex3, splatTexCoord1.st * 1.0) * splatCofac.b * weights.b * 20 +
	// 							  * weights.a * 10;



	float remaining = 1.0 - splatCofac.g;
	vec4 splatDiffuse = texture2D(splatDetailNormalTex3, splatTexCoord0.pq * 1.0) * splatCofac.g;
	if (remaining > 0.0) {
		float maxTake = min(splatCofac.b * 5, remaining);
		splatDiffuse += texture2D(splatDetailNormalTex3, splatTexCoord1.st * 1.0) * maxTake;
		remaining -= maxTake;
	}

	if (remaining > 0.0) {
		float maxTake = min(splatCofac.r * 1, remaining);
		splatDiffuse += texture2D(splatDetailNormalTex1, splatTexCoord0.st * 1.0) * maxTake;
		remaining -= maxTake;
	}

	if (remaining > 0.0) {
		float maxTake = min(splatCofac.a * 4, remaining);
		splatDiffuse += texture2D(splatDetailNormalTex4, splatTexCoord1.pq * 1.0) * maxTake;
		remaining -= maxTake;
	}
	 ...
	 splatDetailStrength.y = clamp(splatDiffuse.a, -1.0, 1.0);
 #endif

	return mix(splatDiffuse, diffuseCol, remaining);


	//float w = (splatCofac.r + splatCofac.g + splatCofac.b + splatCofac.a) / (weights.r + weights.b + weights.a + weights.b);

	// return mix(diffuseCol, splatDiffuse, 2 * w);
	// return diffuseCol;
}
#endif
...
void main() {
	...
	vec4 diffuseCol = texture2D(diffuseTex, diffTexCoords);
		...
		diffuseCol = GetSplatDiffuse(diffuseCol, specTexCoords, splatDetailStrength);
	#endif
